...
import {
  deleteApartmentWithProperty,
  getAllApartmentList,
} from '@/functions/api'
import Loader from '@/components/shared/Loader'
import DeletePropertyConfirmation from '@/components/popup/DeletePropertyConfirmation'
...
const PropertyList = () => {
  ...
  const handleDeleteProperty = async () => {
    await deleteApartmentWithProperty(selectedPropId)
  }
  const redirect = id => {
    router.push(`/admin/marketing/property-list/all-properties/${id}`)
  }
...
const PropertyCard = ({ property, redirectFunc, setOpenPopup }) => {
  return (
    <div
      style={{
        boxShadow:
    ...
    >
      <div className="md:flex gap-4 items-center relative">
        {' '}
        {/* 5 */}
        <div
          onClick={() => redirectFunc(property.id)}
          className="rounded-md cursor-pointer bg-gray-300 p-2 h-full w-full md:w-1/3 flex items-center justify-center overflow-hidden"
        >
          {/* 6 */}
          <img
        ...
        <div className="flex flex-col w-full md:w-2/3 gap-2 mt-2">
          <div className="flex absolute top-2 right-2 justify-center md:justify-end">
            <button
