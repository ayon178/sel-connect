...
import { getPropertyByApartIdAdmin } from '@/functions/api'
...
const AllProperties = ({ params }) => {
  ...
  useEffect(() => {
    ...
    const retriveData = async () => {
      const data = await getPropertyByApartIdAdmin(id)
      setProperties(data)
      setLoading(false)
    }
    retriveData()
  ...
  return (
    <div className=" mx-auto my-10 px-5">
      ...
      <div className="flex max-w-[55rem] focus-within:border-primary mx-auto border border-gray-300 items-center rounded-md ">
        <input
          className="px-4 py-2  text-gray-900 text-sm rounded-lg outline-none block w-full p-2.5"
          type="text"
