...
const PropertyList = () => {
  ...
  useEffect(() => {
    ...
    const fetchProperty = async () => {
      ...
      const propertyList = await getPropertiesByOwnerId(parsed.user_id)
  ...
  return (
    <div className="container mx-auto my-10 px-5">
      ...
      <p className="text-gray-500 mb-5 text-xs md:text-sm">
        All the SEL properties of yours are here
      </p>
      <div className="flex justify-between gap-10 flex-col md:flex-row">
          ...
          <div className="grid grid-cols-1 gap-4">
            {properties.map((property, index) => (
              <div
                key={index}
                onClick={() =>
                  router.push(`/user/my-property/details/${property.id}`)
                }
        ...
        <div className="max-w-[30rem] order-1 md:order-2 hidden md:block">
