...
import ClientFeedRowConst from './ClientFeedRowConst'

const ClientFeedTableConst = ({ tableHeaders, dept, data, setRefetch }) => {
  ...
  return (
    <div
      ...
      className="relative max-w-[300px] mx-auto sm:max-w-[500px] md:max-w-[500px] lg:max-w-[1000px] overflow_custom shadow-md sm:rounded-lg"
    >
      <style>
        {`
          ...
          <tr className="bg-[#FFDED7] text-primary font-semibold">
            {tableHeaders.map((header, index) => (
              <th
                key={index}
                scope="col"
                className={`text-xs sm:text-sm px-2  py-2 sm:py-4 text-left whitespace-nowrap`}
              >
                {header}
              </th>
            ))}
...
export default ClientFeedTableConst
